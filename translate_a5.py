'''
This is the special translate.py for Assignment 5, Task 4.1
'''
...
def main(args):
    ...
    translations = []
    output_lengths = []  # NEW: store token lengths for this run
    ...
    for batch in tqdm(batch_iter(src_encoded, args.batch_size)):
        ...
        for sent in prediction:
            cleaned_ids = postprocess_ids(sent, PAD, BOS, EOS)
            out_len = len(cleaned_ids)
    ...
    if output_lengths:
        ...
        print(f"[ALPHA={args.alpha}] Average output token length: {avg_len:.2f} "
              f"(over {len(output_lengths)} sentences)")
        logging.info(f"[ALPHA={args.alpha}] Average output token length: {avg_len:.2f}")
